 ...
 * Extracted verbatim from radar.html; logic unchanged.
 *
 * NOTE on diag: detectChannel optionally accepts a `diag` counter object so the
 * live scanner can collect funnel stats. It is optional — call detectChannel(candles)
 * with no diag (e.g. in the backtest) and counting is simply skipped. Behavior of the
 * returned channel is identical either way.
 */
...
// --- Confluence signals (computed from daily OHLC) ---
...
function emaState(candles) {
  ...
  var state, pts;
  if(last>e50 && last>e21) { state='above_both'; pts=(e21>=e50)?10:8; }
  else if(last>e50)        { state='above_50';   pts=5; }
...
// --- Channel detection (scoring lives here) ---
// `diag` is optional. When provided, funnel counters are incremented exactly as the
// live scanner expects. When omitted, a no-op stub is used (identical channel result).
function detectChannel(candles, diag) {
  if(!diag) diag = {railPairs:0,posSlope:0,touches:0,containment:0,scoreOk:0};
...
// Node export (browser ignores this; functions remain globals in the browser).
